	...
	<div>
		<form name="login_form" id="login_form" method="POST">
			...
			<table>
				<tr>
					<td>
						<input type="text" name="username" id="username" onfocus="select();" placeholder="Username" value="<?=$username?>"/>
					</td>
					<td>
	...
	<?php } ?>
		...
		<div style="margin: 50px;">
			<div id="header">
				...
				<button type="button" style="<?php echo ($view == 'radar' ? 'border-color: #f60' : ''); ?>" onclick="window.location='index.php?view=radar'">Radar Notifications</button>
			<div style="float: right; padding-right: 50px;"><form method="post"><input type="submit" name="logout" value="Logout"></form></div>
			<div style="float: right; padding-right: 100px;">Logged in as <b><?=$_SESSION['username']?></b></div>
			</div>
			<div id="main">
			<?php
				...
				//SILOS
					...
					$today = date("Y-m-d")."";
					$silos = mysql_query("SELECT * FROM silos INNER JOIN silo_categories USING (silo_cat_id) WHERE end_date < '$today' ORDER BY silo_id");
					$html = "<table id='alternate_table'><tr><th width='3%'></th><th width='5%'>ID #</th><th width='30%'>Silo Name</th><th width='19%'>Category</th><th width='8%'>Type</th><th width='8%'>Admin</th><th width='10%' style='text-align:right'>Goal</th><th width='5%' style='text-align:center'>%</th><th width='5%'>Listings</th><th width='7%' style='text-align:center'>Paid?</th></tr>";
					while ($silo = mysql_fetch_array($silos)) {
						$silo_id = $silo['silo_id'];						
						$admin = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE user_id=".$silo['admin_id']));
						$sql = "SELECT SUM(price) FROM items WHERE deleted_date = 0 AND silo_id = $silo_id AND status = 'Funds Received'";
