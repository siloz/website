...
function ers($ers)
	...
	if (!empty($ers))
		{
		$contents="<table cellspacing='0' cellpadding='0' style='border:1px solid red;' align='center'><tr><td valign='middle' style='padding:3px' valign='middle'><img src='include/OpenInviter/images/ers.gif'></td><td valign='middle' style='color:red;padding:5px;'>";
...
function oks($oks)
	...
	if (!empty($oks))
		{
		$contents="<table border='0' cellspacing='0' cellpadding='10' style='border:1px solid #5897FE;' align='center'><tr><td valign='middle' valign='middle'><img src='include/OpenInviter/images/oks.gif' ></td><td valign='middle' style='color:#5897FE;padding:5px;'>	";
...
if (!$done)
	...
	if ($step=='get_contacts')
		{
		$contents.="<table align='center' class='thTable' cellspacing='5px' cellpadding='5px' style='border:none;'>
			<tr class='thTableRow'><td align='left'><label for='email_box'>Email address</label></td><td><input style='width:200px' type='text' name='email_box' value='{$_POST['email_box']}'></td></tr>
			<tr class='thTableRow'><td align='left'><label for='password_box'>Password</label></td><td><input style='width:200px' type='password' name='password_box' value='{$_POST['password_box']}'></td></tr>
			<tr class='thTableRow'><td align='left'><label for='provider_box'>Email provider</label></td><td><select style='width:200px' name='provider_box'><option value=''></option>";
		...
		$contents.="</select></td></tr>
			<tr class='thTableImportantRow'><td colspan='2' align='center'><input class='button' type='submit' name='import' value='Import Contacts'></td></tr>
		...
		$contents.="<table class='thTable' cellspacing='0' cellpadding='0' style='border:none;'>
				<tr class='thTableRow'><td align='right' valign='top'><label for='message_box'><b>Message: </b></label></td><td><textarea rows='5' cols='50' name='message_box' class='thTextArea' style='width:300px;'>{$_POST['message_box']}</textarea></td></tr>
...
if (!$done)
	...
	if ($step=='send_invites')
		...
		if ($inviter->showContacts())
			{
			$contents.="<table class='thTable' align='center'  border='1px' cellspacing='3px' cellpadding='3px'><tr class='thTableHeader'><td colspan='".($plugType=='email'? "3":"2")."' align='center'><b>Your contacts of ".$_POST['email_box']."</b></td></tr>";
			if (count($contacts)==0)
				...
				{
				$contents.="<tr class='thTableDesc'><td><input type='checkbox' onChange='toggleAll(this)' name='toggle_all' title='Select/Deselect all' checked><b>Invite?</b></td><td><b>Name</b></td>".($plugType == 'email' ?"<td><b>Email</b></td>":"")."</tr>";
				$odd=true;$counter=0;
